...
describe Api::V4::JobsController do 
  ...
  describe "POST /api/v4/jobs/decline" do

    context "when job is requsted." do
      it "should decline job." do
        post decline_api_v4_jobs_path, { "phone" => job_seeker.phone, job_id: job.id },{ "HTTP_AUTHORIZATION" => client.api_key }
    ...
    context "with invalid api_key" do
      it "should error about invalid api key." do
        post decline_api_v4_jobs_path, { "phone" => job_seeker.phone, job_id: job.id },{ "HTTP_AUTHORIZATION" => 'invalid_key' }
        json = JSON.parse(response.body)
    ...
    context "without phone" do
      it "should prompt for phone required." do
        post decline_api_v4_jobs_path, { job_id: job.id },{ "HTTP_AUTHORIZATION" => client.api_key }
        json = JSON.parse(response.body)
     ...
     context "when phone doesn't belong to any job seeker" do
      it "should error about job seeker not found." do
        post decline_api_v4_jobs_path, { "phone" => 'wrong_number', job_id: job.id },{ "HTTP_AUTHORIZATION" => client.api_key }
        json = JSON.parse(response.body)
     ...
     context "when job is not specified" do
      it "should prompt for a job." do
        post decline_api_v4_jobs_path, { "phone" => job_seeker.phone },{ "HTTP_AUTHORIZATION" => client.api_key }
        json = JSON.parse(response.body)
